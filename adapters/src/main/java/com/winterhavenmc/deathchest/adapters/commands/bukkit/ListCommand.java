...
final class ListCommand extends SubcommandAbstract
{
	 ...
	 * Class constructor
	 */
	ListCommand(final CommandCtx ctx)
	{
		this.ctx = ctx;
		this.name = "list";
	                                  ...
	                                  final String[] args)
	{
		...
		if (args.length == 2)
		{
			if (sender.hasPermission("deathchest.list.other"))
			{
				...
				for (ValidDeathChest validDeathChest : ctx.chestManager().getAllChests())
				{
					chestOwners.put(validDeathChest.ownerUid(),
							ctx.plugin().getServer().getOfflinePlayer(validDeathChest.ownerUid()).getName());
				}
	...
	public boolean onCommand(final CommandSender sender, final List<String> args)
	{
		...
		{
			ctx.messageBuilder().compose(sender, MessageId.COMMAND_FAIL_LIST_PERMISSION).send();
			return true;
		}
		...
		if (passedPlayerName.isEmpty())
		{
			...
			{
				displayRecords = new ArrayList<>(ctx.chestManager().getAllChests());
			}
		}
		else
		{
			...
			{
				return ctx.messageBuilder().compose(sender, MessageId.COMMAND_FAIL_LIST_OTHER_PERMISSION).send();
			}
		}
	                                 ...
	                                 final boolean displayNames)
	{
		...
		{
			return ctx.messageBuilder().compose(sender, MessageId.LIST_EMPTY).send();
		}
		...
		int itemsPerPage = ctx.plugin().getConfig().getInt("list-page-size-player");
		if (sender instanceof ConsoleCommandSender)
		{
			itemsPerPage = ctx.plugin().getConfig().getInt("list-page-size-console");
		}
		...
		for (ValidDeathChest deathChest : displayRange)
		{
			...
			if (displayNames)
			{
				ctx.messageBuilder().compose(sender, MessageId.LIST_ITEM_ALL)
	...
	{
		return ctx.chestManager().getAllChests().stream()
