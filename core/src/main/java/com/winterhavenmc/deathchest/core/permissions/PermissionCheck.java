...
final public class PermissionCheck
{
	 ...
	 * Class constructor
	 */
	public PermissionCheck(final ListenerCtx ctx)
	{
		this.ctx = ctx;
	}
	...
	public void logDebugMessage(final String message)
	{
		...
		{
			ctx.plugin().getLogger().info(message);
		}
	}
	...
	public boolean isCreativeDeployDisabled(final Player player)
	{
		return player.getGameMode().equals(GameMode.CREATIVE)
				&& !ctx.plugin().getConfig().getBoolean("creative-deploy")
	...
	{
		return player.getGameMode().equals(GameMode.CREATIVE)
				&& !ctx.plugin().getConfig().getBoolean("creative-access")
	 ...
	 *
	 * @param validDeathChest the deathchest to check
	 * @return true if chest is already open, false if not
	 */
	private boolean isCurrentlyOpen(final ValidDeathChest validDeathChest)
	{
		return ctx.chestManager().getViewerCount(validDeathChest) > 0;
	}
	...
	{
		return !ctx.plugin().getConfig().getBoolean("chest-protection");
	}
	 ...
	 */
	private boolean isProtectionExpired(final ValidDeathChest deathChest)
	{
		return ctx.plugin().getConfig().getBoolean("chest-protection") &&
				ctx.chestManager().protectionExpired(deathChest);
	}
	 ...
	 *
	 * @param validDeathChest the death chest to check expiration
	 * @return true if protection is enabled and chest has not expired, otherwise false
	 */
	private boolean isProtectionNotExpired(final ValidDeathChest validDeathChest)
	{
		return ctx.plugin().getConfig().getBoolean("chest-protection") &&
	 ...
	 */
	private boolean isKillerLooting(final Player player, final ValidDeathChest deathChest)
	{
		return ctx.plugin().getConfig().getBoolean("chest-protection") &&
				ctx.plugin().getConfig().getBoolean("killer-looting") &&
				deathChest.killerUid().equals(player.getUniqueId()) &&
	 ...
	 * @param validDeathChest the deathchest that is being quick-looted
	 */
	public void performChecks(final Cancellable event, final Player player,
	                          final ValidDeathChest validDeathChest, final ResultAction resultAction)
	{

		// get protectionCheckResult of all protection plugin checks
		final ProtectionCheckResult protectionCheckResult = ctx.protectionPluginRegistry().accessAllowed(player, validDeathChest.getLocation());
		...
		if (isCurrentlyOpen(validDeathChest))
		{
			...
			ctx.chestManager().getInventory(validDeathChest).ifPresent(inventory ->
			{
				String viewerName = inventory.getViewers().getFirst().getName();
				ctx.messageBuilder().compose(player, MessageId.CHEST_CURRENTLY_OPEN)
						.setMacro(Macro.DEATH_CHEST, validDeathChest)
						.setMacro(Macro.VIEWER, viewerName)
						.send();
				ctx.messageBuilder().sounds().play(player, SoundId.CHEST_DENIED_ACCESS);
			});
			return;
		}
		...
		if (isCreativeAccessDisabled(player))
		{
			event.setCancelled(true);
			ctx.messageBuilder().sounds().play(player, SoundId.CHEST_DENIED_ACCESS);
			ctx.messageBuilder().compose(player, MessageId.NO_CREATIVE_ACCESS)
		...
		{
			resultAction.execute(event, player, validDeathChest, ctx.chestManager());
			return;
		}
		...
		if (isProtectionExpired(validDeathChest))
		{
			resultAction.execute(event, player, validDeathChest, ctx.chestManager());
			return;
		}
		...
		if (isKillerLooting(player, validDeathChest))
		{
			resultAction.execute(event, player, validDeathChest, ctx.chestManager());
			return;
		}
		...
		if (isProtectionNotExpired(validDeathChest))
		{
			ctx.messageBuilder().compose(player, MessageId.CHEST_ACCESSED_PROTECTION_TIME)
					.setMacro(Macro.DEATH_CHEST, validDeathChest)
					.send();
		}
		...
		{
			ctx.messageBuilder().compose(player, MessageId.NOT_OWNER)
					.setMacro(Macro.DEATH_CHEST, validDeathChest)
					.send();
		}
