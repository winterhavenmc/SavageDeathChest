...
public record ChestSign(ListenerCtx ctx, Player player, ValidDeathChest deathChest)
{
	public void place()
	{
		// if chest-signs are not enabled in configuration, do nothing and return
		if (!ctx.plugin().getConfig().getBoolean("chest-signs"))
		{
			return;
		}
		...
		else if (!ctx.plugin().getConfig().getBoolean("holograms-enabled"))
		{
	...
	private void finalizeSign(final Block signBlock, final Player player, final ValidDeathChest validDeathChest)
	{
		...
		if (chestBlock instanceof SignChestBlock signChestBlock)
		{
			...
			ctx.chestManager().putBlock(ChestBlockType.SIGN, signChestBlock);

			// set sign block metadata
			ctx.chestManager().setMetadata(signChestBlock, validDeathChest);
		}
	}
	...
	private void setSignText(final Block signBlock, final Player player)
	{
		...
		String dateFormat = ctx.plugin().getConfig().getString("DATE_FORMAT");
		...
		Locale locale = Locale.forLanguageTag(LanguageTag.of(ctx.plugin().getConfig().getString("locale"))
				.orElse(LanguageTag.of(ctx.plugin().getConfig().getString("language"))
		...
		List<String> lines = ctx.plugin().getConfig().getStringList("SIGN_TEXT");
		...
		for (String line : lines)
		{
			...
			line = line.replace("{WORLD}", ctx.messageBuilder().worlds().aliasOrName(player.getWorld().getUID()).orElse("???"));
	...
	private boolean isValidSignLocation(final Location location)
	{
		...
		return ctx.chestManager().isReplaceableBlock(block);
	}
