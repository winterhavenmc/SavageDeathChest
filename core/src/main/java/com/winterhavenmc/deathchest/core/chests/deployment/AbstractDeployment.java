...
{
	protected final ListenerCtx ctx;
	 ...
	 */
	public AbstractDeployment(final ListenerCtx ctx, final Player player, final Collection<ItemStack> droppedItems)
	{
		this.ctx = ctx;
		this.player = player;
	...
	void placeChest(final ValidDeathChest deathChest, final Location location, final ChestBlockType chestBlockType)
	{
		...
		if (chestBlock instanceof ValidChestBlock validChestBlock)
		{
			...
			ctx.chestManager().putBlock(chestBlockType, validChestBlock);

			// set block metadata
			ctx.chestManager().setMetadata(validChestBlock, deathChest);
		}
	}
	...
	{
		return ctx.plugin().getConfig().getBoolean("require-chest")
	...
	private void setCustomInventoryName(final Player player, final Block block)
	{
		...
		ctx.messageBuilder().constants().getString(inventoryNameKey).ifPresent(string -> {
	...
	void finalize(final SearchResult searchResult, final ValidDeathChest validDeathChest)
	{
				...
				&& !searchResult.getResultCode().equals(SearchResultCode.PARTIAL_SUCCESS))
		{
			...
			ctx.chestManager().cancelExpireTask(validDeathChest);
			return;
		}

		// get configured chest protection time
		long chestProtectionTime = ctx.plugin().getConfig().getLong("chest-protection-time");
		...
		if (ctx.plugin().getConfig().getBoolean("chest-protection") && chestProtectionTime > 0)
		{
			ctx.messageBuilder().compose(player, MessageId.CHEST_DEPLOYED_PROTECTION_TIME)
					.setMacro(Macro.DEATH_CHEST, validDeathChest)
					.send();
		}

		// put DeathChest in DeathChest map
		ctx.chestManager().putChest(validDeathChest);

		// put DeathChest in datastore
		Set<ValidDeathChest> deathChests = Collections.singleton(validDeathChest);
		ctx.chestManager().saveChestRecords(deathChests);
	}
	...
	private void sendResultMessage(final Player player, final ValidDeathChest validDeathChest, final SearchResult result)
	{
		...
		{
			case SUCCESS -> ctx.messageBuilder().compose(player, MessageId.CHEST_SUCCESS)
					.setMacro(Macro.DEATH_CHEST, validDeathChest)
					.send();
			case PARTIAL_SUCCESS -> ctx.messageBuilder().compose(player, MessageId.DOUBLECHEST_PARTIAL_SUCCESS)
					.setMacro(Macro.DEATH_CHEST, validDeathChest)
					.send();
			case PROTECTION_PLUGIN -> ctx.messageBuilder().compose(player, MessageId.CHEST_DENIED_DEPLOYMENT_BY_PLUGIN)
			...
			case ABOVE_GRASS_PATH, NON_REPLACEABLE_BLOCK ->
					ctx.messageBuilder().compose(player, MessageId.CHEST_DENIED_BLOCK)
							.setMacro(Macro.LOCATION, result.getLocation())
							.send();
			case ADJACENT_CHEST -> ctx.messageBuilder().compose(player, MessageId.CHEST_DENIED_ADJACENT)
					.setMacro(Macro.LOCATION, result.getLocation())
					.send();
			case NO_REQUIRED_CHEST -> ctx.messageBuilder().compose(player, MessageId.NO_CHEST_IN_INVENTORY)
					.setMacro(Macro.LOCATION, result.getLocation())
					.send();
			case SPAWN_RADIUS -> ctx.messageBuilder().compose(player, MessageId.CHEST_DENIED_SPAWN_RADIUS)
					.setMacro(Macro.LOCATION, result.getLocation())
					.send();
			case VOID -> ctx.messageBuilder().compose(player, MessageId.CHEST_DENIED_VOID)
					.setMacro(Macro.LOCATION, result.getLocation())
					.send();
		}
	}
	...
	private void logResult(final SearchResult result)
	{
		if (result == null)
		{
			ctx.plugin().getLogger().info("SearchResult is null!");
			return;
		}

		if (result.getResultCode() != null)
		{
			ctx.plugin().getLogger().info("SearchResult Code: " + result.getResultCode());
		}

		if (result.getLocation() != null)
		{
			ctx.plugin().getLogger().info("Location: " + result.getLocation());
		}

		if (result.getProtectionPlugin() != null)
		{
			ctx.plugin().getLogger().info("Protection Plugin: " + result.getProtectionPlugin());
		}

		if (result.getRemainingItems() != null)
		{
			ctx.plugin().getLogger().info("Remaining Items: " + result.getRemainingItems());
		}
	}
