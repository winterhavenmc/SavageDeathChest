...
package com.winterhavenmc.deathchest.core.chests;
...
public final class ChestManager
{
	 ...
	 * Class constructor
	 */
	public ChestManager(final Plugin plugin, final MessageBuilder messageBuilder, final ConnectionProvider connectionProvider)
	{
		this.plugin = plugin;
		replaceableBlocks = new ReplaceableBlocks(plugin);
		dataStore = DataStore.connect(plugin, connectionProvider);
		this.messageBuilder = messageBuilder;
		chestMap = new ChestMap();
	...
	{
		this.chestMap.put(plugin, this, validDeathChest);
	}
	...
	{
		return this.chestMap.get(chestUid);
	}
	...
	public DeathChest getChest(final Inventory inventory)
	{
		...
		if (isDeathChestInventory(inventory) && inventory.getHolder() instanceof Chest chest)
		{
	...
	{
		this.chestMap.remove(validDeathChest);
	}
	...
	{
		return this.blockMap.getBlocks(chestUid);
	}
	...
	{
		return this.blockMap.getBlockMap(chestUid);
	}
	...
	{
		this.blockMap.remove(validChestBlock);
	}
	...
	public boolean isChestBlock(final Block block)
	{
		return block != null
				...
				&& this.blockMap.containsKey(block.getLocation());
	}
	...
	public boolean isChestBlockChest(final Block block)
	{
		return block != null
				...
				&& blockMap.containsKey(block.getLocation());
	}
	...
	{
		return chestMap.getExpireTaskId(validDeathChest);
	}
	...
	public Optional<Inventory> getInventory(Map<ChestBlockType, ValidChestBlock> blockMap)
	{
		...
		Optional<Inventory> rightChestInventory = getInventory(blockMap.get(ChestBlockType.RIGHT_CHEST));

		return rightChestInventory.isEmpty()
				? getInventory(blockMap.get(ChestBlockType.LEFT_CHEST))
				: rightChestInventory;
	}
	...
	public void expire(final ValidDeathChest validDeathChest)
	{
		...
		if (player != null)
		{
			messageBuilder.compose(player, MessageId.CHEST_EXPIRED)
	...
	public void destroy(final ValidDeathChest validDeathChest)
	{
		...
		messageBuilder.sounds().play(validDeathChest.getLocation(), SoundId.CHEST_BREAK);

		// get block map for this chest
		Map<ChestBlockType, ValidChestBlock> chestBlockMap = getBlockMap(validDeathChest.chestUid());
	...
	public void autoLoot(final Player player, final ValidDeathChest validDeathChest)
	{
		...
		if (player != null)
		{
			...
			{
				remainingItems.addAll(transferContents(validChestBlock, player));
			}

			// if remainingItems is empty, all chest items fit in player inventory so destroy chest and return
			if (remainingItems.isEmpty())
			{
				destroy(validDeathChest);
				return;
			}

			// send player message
			messageBuilder.compose(player, MessageId.INVENTORY_FULL)
					.setMacro(Macro.LOCATION, player.getLocation())
					.send();

			// try to put remaining items back in chest
			remainingItems = fill(remainingItems, validDeathChest);
	...
	public Optional<Block> getAttachedBlock(final Block block)
	{
		...
		return isChestBlockChest(attached)
	...
	Optional<Inventory> getInventory(final ValidChestBlock validChestBlock)
	{
		...
		{
			// get attached block
			Optional<Block> attachedBlock = getAttachedBlock(validChestBlock.getLocation().getBlock());
		...
		if (isChestBlockChest(block))
		{
			...
			for (ItemStack itemStack : chestInventory)
			{
				...
				if (itemStack != null)
				{
					...
					messageBuilder.sounds().play(player, SoundId.INVENTORY_ADD_ITEM);
				}
			}
		}
		return remainingItems;
	...
	void destroy(final ValidChestBlock validChestBlock)
	{
		if (validChestBlock.getLocation() != null)
		{
			...
			removeMetadata(validChestBlock);

			// remove validChestBlock record from datastore
			deleteBlockRecord(validChestBlock);

			// remove validChestBlock from block map
			removeBlock(validChestBlock);
