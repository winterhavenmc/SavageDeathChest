...
public final class SqliteSchemaUpdaterV1 implements SqliteSchemaUpdater
{
	...
	SqliteSchemaUpdaterV1(final Plugin plugin,
	...
	private Set<ValidDeathChest> selectAllChestRecords(final Plugin plugin, final Connection connection) throws SQLException
	{
		...
		try (PreparedStatement preparedStatement = connection.prepareStatement(SqliteQueries.getQuery("SelectAllChestsV1")))
		{
			...
			while (resultSet.next())
			{
				...
				OfflinePlayer offlineOwner = plugin.getServer().getOfflinePlayer(ownerUid);
				String ownerName = (offlineOwner.getName() != null)
						? offlineOwner.getName()
						: "\uD83D\uDC64";

				OfflinePlayer offlineKiller = plugin.getServer().getOfflinePlayer(killerUid);
				String killerName = (offlineKiller.getName() != null)
